...
def make_turn(data: dict) -> BattleOutput:
    ...
    for ship in battle_state.My:
        ...
        x_our, y_our, z_our = ship.Position
        distance_to_enemy_ship = []
        for ship_opponent in battle_state.Opponent:
            x, y, z = ship_opponent.Position
            distance_to_enemy_ship.append({
                'coor_opponent': f'{x}/{y}/{z}',
                'distance': sqrt((x - x_our) ** 2 + (y - y_our) ** 2 + (z - z_our) ** 2),
                'opponent_ship': ship_opponent
            })
        ...
        for distance in distance_to_enemy_ship:
            ...
            # добавил поиск самого мощного оружия, которое может достерлять(достать до противника)
            guns = list(filter(lambda g: g.Radius >= distance['distance'], guns))
            guns.sort(key=lambda g: g.Damage)
            # ------------------------------
            friendly_fire = False
            # TODO: Сделать проверку на стрельбу по своим

            if guns and (not attack_was) and \
                    (not friendly_fire):  # если ни одно оружие не достает до корабля, то guns будет пуст
                gun = guns[0]
                battle_output.UserCommands.append(UserCommand(
                    Command='ATTACK',
                    Parameters=AttackCommandParameters(ship.Id, gun.Name, distance['opponent_ship'].Position)
                ))

                attack_was = True

        nearest_enemy = distance_to_enemy_ship[0]['opponent_ship']  # возвращает ближайший вражеский корабль

        if not attack_was:
            battle_output.UserCommands.append(UserCommand(
                Command='MOVE',
                Parameters=MoveCommandParameters(ship.Id, nearest_enemy.Position)
            ))
